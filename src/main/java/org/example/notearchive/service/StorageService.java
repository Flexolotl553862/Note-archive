...
public class StorageService {
    ...
    public StorageService(@Value("${storage.path}") String storagePath, StorageEntryRepository storageEntryRepository) {
        this.storagePath = storagePath;
        this.storageEntryRepository = storageEntryRepository;
    }

    public File getEntryContent(long id) throws StorageException {
        StorageEntry entry = storageEntryRepository
                .findById(id)
                .orElseThrow(() -> new StorageException("File not found", null));
    ...
    public void uploadMultipartFile(MultipartFile file, StorageEntry root, boolean saveAsNestedDirectories)
            throws StorageException {

        if (!saveAsNestedDirectories || !tryToSaveAsZIP(file, root)) {
            try {
                saveAsFile(file.getInputStream(), file.getOriginalFilename(), root);
            } catch (IOException e) {
    ...
    public void addMarkdownDescription(Note note, String markdownDescription) throws StorageException {
        saveAsFile(new ByteArrayInputStream(markdownDescription.getBytes()), "description.md", note.getContent());
        storageEntryRepository.save(note.getContent());
    }

    public void deleteEntryById(long id) throws StorageException {
        StorageEntry entry = storageEntryRepository
                .findById(id)
                .orElseThrow(() -> new StorageException("File not found", null));
    ...
    public void createDirectory(String name, long parentId) throws StorageException {
        createDirectory(name, storageEntryRepository.findById(parentId).orElseThrow());
    }
    ...
    private void saveAsFile(InputStream data, String name, StorageEntry root) throws StorageException {
        try {
            Path filePath = Path.of(storagePath, root.getPath(), name);
            Files.createDirectories(filePath.getParent());
            data.transferTo(new FileOutputStream(Path.of(storagePath, root.getPath(), name).toFile()));
            StorageEntry entry = new StorageEntry(
                    name,
    ...
    private boolean tryToSaveAsZIP(MultipartFile file, StorageEntry root)
            throws StorageException {
        ZipInputStream zis;
        Tika tika = new Tika();
        try {
